...
function Cart() {
  ...
  const { postProductCar, addCard ,getProductCar,deleteProductCar,updateProductCar} = useContextShopCar();
  ...
  const UpdateProductCar = async (id, amount, datas)=>{
    const data ={
      amount,
      code:datas.id_store
    }
     ...
     <div class="mx-auto max-w-5xl justify-center px-6 md:flex md:space-x-6 xl:px-0">
          ...
          addCard.map(i=> (
           ...
           <div class="sm:ml-4 sm:flex sm:w-full sm:justify-between">
             <div class="mt-5 sm:mt-0">
               <h2 class="text-lg font-bold text-gray-900">{i.name_product}</h2>
               <p class="mt-1 text-xs text-gray-700">{i.description_product}</p>
             </div>
             <div class="mt-4 flex justify-between im sm:space-y-6 sm:mt-0 sm:block sm:space-x-6">
               <div class="flex items-center border-gray-100">
                ...
                <span className="mt-2 block">{i.amount_Product}</span>
                 ...
                 <span class="cursor-pointer rounded-r bg-gray-100 py-1 px-3 duration-100 hover:bg-blue-500 hover:text-blue-50"onClick={()=> UpdateProductCar(i.id_product, 1, i)}> + </span>
               </div>
               <div class="flex items-center space-x-4">
                 <p class="text-sm">{money.format(i.price_product)}</p>
                 <div className="absolute top-2 right-3 bg-gray-100 rounded-full p-1" onClick={()=>DeleteProductCar(i.id_product)}>
       ...
       <div class="mt-6 h-full rounded-lg border bg-white p-6 shadow-md md:mt-0 md:w-1/3 sticky top-[5rem]">
         <div class="mb-2 flex justify-between">
           ...
           <p class="text-gray-700 flex flex-col">{addCard.map(i=> (
            <span>{money.format(i.price_product*i.amount_Product)}</span>
           ))}</p>
         </div>
